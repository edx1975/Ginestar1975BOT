...
from kinship_weights import prioritize_relationships, calculate_path_kinship_score, calculate_genealogical_importance
...
/qui_soc - Mostra la teva identitat actual
/relacio amb <nom> - Calcula la relació amb una persona
/grup - Mostra les teves relacions amb el grup
/kintos - Mostra matriu de relacions entre tots els Kintos
/apodos - Mostra tots els apodos disponibles

*Exemples:*
• `/relacio amb Edu` - Calcula la relació amb Edu (apodo d'Eduard)
• `/relacio amb Mireia` - Calcula la relació amb Mireia
• `/relacio amb Montse` - Calcula la relació amb Montse (apodo de Montserrat)
        """
    ...
    async def relacio_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            await update.message.reply_text(
                "❌ Has d'especificar el nom de la persona.\n\n"
                "Exemple: `/relacio amb Eduard`"
            )
            return
        ...
        if relacions_sanguinies:
            ...
            text = f"🩸 *Relacions sanguínies amb {persona_trobada['nom']}:*\n\n"
        else:
        ...
        if len(relacions_a_mostrar) > 1 or len(text) > 3000:
            ...
            for i, relacio in enumerate(relacions_a_mostrar):
                ...
                # Calcular scores per aquesta relació
                kinship_score, rel_type, path_weights = calculate_path_kinship_score(relacio.cami, self.persones)
                importance_score = calculate_genealogical_importance(relacio, self.persones)
                ...
                relacio_text += f"• **Pes:** {importance_score:.0f}\n"
            ...
            for i, relacio in enumerate(relacions_a_mostrar):
                ...
                text += f"• **Pes:** {importance_score:.0f}\n"
    ...
    async def grup_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Calcular relacions de l'usuari amb la resta del grup
        relacions = []
        for persona in persones:
            if persona["id"] != usuari.persona_id:  # No calcular relació amb un mateix
                relacions_totes = self._calcular_totes_relacions(usuari.persona_id, persona["id"])
                if relacions_totes:
                    # Agafar la millor relació (la primera després de prioritzar)
                    relacions.append(relacions_totes[0])
        
        if not relacions:
            await update.message.reply_text("❌ No s'han pogut calcular relacions.")
            return
        
        # Organitzar per distància
        relacions_ordenades = sorted(relacions, key=lambda r: r.distancia)
        
        # Mostrar resultats
        text = f"🧬 *Les teves relacions amb el grup Kintos:*\n\n"
        
        for relacio in relacions_ordenades:
            persona_altra = self.persones[relacio.id2].nom
            emoji = "🩸" if relacio.tipus == "sanguinia" else "💍"
            
            # Calcular importància genealògica
            importancia = calculate_genealogical_importance(relacio, self.persones)
            
            text += f"{emoji} *{persona_altra}*\n"
            text += f"   {relacio.grau} (distància: {relacio.distancia} | Pes: {importancia:.0f})\n\n"
    ...
    async def kintos_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Calcular totes les relacions entre Kintos
        relacions = {}
        for i, persona1 in enumerate(persones):
            for j, persona2 in enumerate(persones):
                if i != j:  # No calcular relació amb un mateix
                    relacio = self._calcular_relacio(persona1["id"], persona2["id"])
                    if relacio:
                        relacions[f"{persona1['id']}-{persona2['id']}"] = relacio
        ...
        for persona in persones:
            ...
            # Trobar totes les relacions d'aquesta persona
            relacions_persona = []
            for relacio in relacions.values():
                if relacio.id1 == persona_id:
                    relacions_persona.append(relacio)
            
            if relacions_persona:
                # Ordenar per distància
                relacions_persona.sort(key=lambda r: r.distancia)
                
                text += f"👤 *{persona_nom}:*\n"
                for relacio in relacions_persona:
                    persona_altra = self.persones[relacio.id2].nom
                    emoji = "🩸" if relacio.tipus == "sanguinia" else "💍"
                    text += f"  {emoji} {persona_altra} ({relacio.grau}, dist:{relacio.distancia})\n"
                text += "\n"
        
        # Dividir el missatge si és massa llarg
        if len(text) > 4000:
            # Enviar missatge inicial
            await update.message.reply_text("🧬 *Matriu de relacions Kintos*\n\n*Carregant...*")
            ...
            for persona in persones:
                ...
                if relacions_persona:
                    ...
                    persona_text = f"👤 *{persona_nom}:*\n"
                    for relacio in relacions_persona:
                        persona_altra = self.persones[relacio.id2].nom
                        emoji = "🩸" if relacio.tipus == "sanguinia" else "💍"
                        persona_text += f"  {emoji} {persona_altra} ({relacio.grau}, dist:{relacio.distancia})\n"
    ...
    def _calcular_totes_relacions(self, id1: str, id2: str) -> List[Relacio]:
        ...
        if tots_camins:
            ...
            # Utilitzar el sistema de pesos genealògics per prioritzar i combinar relacions
            relacions_prioritzades = prioritize_relationships(relacions_filtrades, self.persones)
            
            return relacions_prioritzades
        
        # Si no n'hi ha, buscar tots els camins i prioritzar
        try:
            tots_camins = list(nx.all_simple_paths(self.graph_builder.graph, id1, id2, cutoff=15))
        except nx.NetworkXNoPath:
            return []
        ...
        # Filtrar camins duplicats o amb loops innecessaris
        relacions_filtrades = self._filtrar_camins_duplicats(relacions)
        
        # Utilitzar el sistema de pesos genealògics per prioritzar i combinar relacions
        relacions_prioritzades = prioritize_relationships(relacions_filtrades, self.persones)
        
        return relacions_prioritzades[:3]
    ...
    def _filtrar_camins_duplicats(self, relacions: List[Relacio]) -> List[Relacio]:
        ...
        # Per cada grup, seleccionar els millors camins
        resultat = []
        for clau, grup_relacions in relacions_per_grup.items():
            ...
            # Per relacions no sanguínies, agafar fins a 3 del mateix grup
            # Per relacions sanguínies, agafar totes
            if clau[1] == "no_sanguinia":
                # Agafar fins a 3 del mateix grup de distància
                resultat.extend(grup_relacions[:3])
            else:
                # Agafar totes les sanguínies
                resultat.extend(grup_relacions)
        
        # Ordenar per distància (més properes primer)
        resultat.sort(key=lambda r: r.distancia)
